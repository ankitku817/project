    ...
    def sign_up_database(self):  
        if self.emailEntry.get() ==''or self.userEntry.get()=='' or self.cpassEntry.get()=='':
            ...
            try:
                ...
                query='use train'
                mycursor.execute(query)
                query='create table data(id int auto_increment primary key not null,email varchar(30) unique key,username varchar(30),password varchar(30))'
                mycursor.execute(query)
            except:
...
#-------------------------------------------------------Function For Login Page Import Login page-----------------------------------------------------------#
    def login_page(self):
        root.destroy()
        import Passenger_Login_file
