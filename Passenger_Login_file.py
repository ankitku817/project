...
bgImage=ImageTk.PhotoImage(file="tra1.jpg")
bgLable=Label(image=bgImage)
bgLable.place(x=0,y=0)

g1Image=ImageTk.PhotoImage(file="tra2.jpg")
g1Lable=Label(image=g1Image,bd=0,background='green')
g1Lable.place(x=1030,y=0)

#-------------------------------------------------------------Create A Class Named As Sign_in_page ---------------------------------------------------
...
#-----------------------------------------------------------------Function For Change Password--------------------------------------------------------
    def change_pass(self):
        if self.userEntry.get()=='' or self.passEntry.get()=='' or  self.cpassEntry.get()=='':
            ...
            mycursor=con.cursor()
            query='select * from data where self.username=%s'
    ...
    def signin_user(self):
        if self.username.get()=='' or self.userpass.get()=='':
            ...
            if row==None:
                ...
                root.destroy()
                import Passenger_page

            
                             
    def signup_page(self):
        root.destroy()
        import Passenger_SignUp_file
...
#---------------------------------------------------------------------Function For Log__in_page-----------------------------------------------------------#
    def Log__in_page(self):
        ...
        self.loginButton.place(x=55,y=200)
        ...
        self.newaccount=Label(self.f1,text='Dont have any account ->',font=('fantasty',12,'bold'),bg='blue',fg='white',)
        self.newaccount.place(x=5,y=360)

        self.newaccountButton=Button(self.f1,text='Create new account',font=('Open sans',14,'bold'),bg='blue',fg='white',bd=0,activebackground='blue',activeforeground='white',cursor='hand2',command=self.signup_page)
        self.newaccountButton.place(x=205,y=358)
